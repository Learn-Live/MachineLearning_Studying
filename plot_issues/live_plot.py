...
def plot_hierarchy(X, y):
    r"""
        Matplotlib Object Hierarchy
            Figure->(multiple)Axes-> indiviual plot_issues -> trick marks, line, legends, ..
    ...
    """
        when we call plot_issues(), matplotlib calls gca() to get the current axes and gca in turn calls gcf() to get 
...
def live_plot(X, y):
    ...
    #     # line,=ax_i.plot_issues(X, y, animated=True)
    ...
    def update(frame_data):
        ...
        for ax_i in axes.flatten():
            ...
            # ax_i.plot_issues(X, y, animated=True)
            ax_i.set_xlabel('weight')
            ax_i.set_ylabel('Frequency')
            ax_i.set_xlim(-100, 10000)
            ax_i.set_ylim(-100, 10000)
...
def live_plot_2(X,y):
    r"""
        refer to:
            https://stackoverflow.com/questions/9401658/how-to-animate-a-scatter-plot_issues
...
def dynamic_plot(X, y, title='dynamic_plot'):
    ...
    #     # plt.scatter(X, y)
    #     plt.plot_issues(X,y,'k-o')
    #     plt.xlim(0,100)
    #     plt.ylim(0,100)

    with writer.saving(fig, "writer_test.mp4", dpi=100):
        for k in range(10):
            # Create a new plot_issues object
            ax = plt.scatter(X[:k * 2], y[:k * 2])
            # update_figure(X,y)
            # ax = plt.plot_issues(X,y)
            writer.grab_frame()
...
#             xar.append(variables_n_data_types_issues(x))
#             yar.append(variables_n_data_types_issues(y))
#     ax1.clear()
#     ax1.plot_issues(xar, yar)
#
# anim = animation.FuncAnimation(fig, animate, interval=50, blit=True)
# plt.show()
# anim.save('noise.gif', writer='ffmpeg', fps=10, dpi=100, metadata={'title':'TexShop modify Syntax colors.md'})
#
#
...
# axes.set_xlim(0, 100)
# axes.set_ylim(-50, +50)
# line, = axes.plot_issues(xdata, ydata, 'r-')
#
# for i in range(100):
#     xdata.append(i)
...
# fig, ax = plt.subplots()
# xdata, ydata = [], []
# ln, = plt.plot_issues([], [], 'ro', animated=True)
#
# def init():
...
# from python3_issues.tricks import load_data
